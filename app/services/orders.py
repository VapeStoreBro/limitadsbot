...
async def get_price(session: AsyncSession, user_id: int, tariff_code: str, duration_code: str) -> tuple[int, int, int | None]:
    custom = await session.scalar(select(UserPrice).where(
        UserPrice.user_id == user_id,
        UserPrice.tariff_code == tariff_code,
        UserPrice.duration_code == duration_code,
    ))
    base = await session.scalar(select(TariffPrice).where(
...
async def prices_for_user(session: AsyncSession, user_id: int, tariff_code: str) -> dict[str, int]:
    base_rows = (await session.scalars(select(TariffPrice).where(TariffPrice.tariff_code == tariff_code))).all()
    result = {row.duration_code: row.price_rub for row in base_rows}
    custom_rows = (await session.scalars(select(UserPrice).where(
        UserPrice.user_id == user_id,
        UserPrice.tariff_code == tariff_code,
    ))).all()
...
async def slot_available(session: AsyncSession, tariff_code: str, start_at: datetime, end_at: datetime, ignore_order_id: int | None = None) -> bool:
    ...
    conditions = [
        ...
        AdOrder.status.in_([OrderStatus.BOOKED.value, OrderStatus.READY.value, OrderStatus.ACTIVE.value]),
    ...
    count = await session.scalar(select(func.count()).select_from(AdOrder).where(and_(*conditions)))
    return int(count or 0) < limit


async def create_order(session: AsyncSession, *, user_id: int, tariff_code: str, duration_code: str, content_text: str, media: list[dict], buttons: list[dict], requested_start_at: datetime | None) -> AdOrder:
    price, hours, _ = await get_price(session, user_id, tariff_code, duration_code)
    now = datetime.now(timezone.utc)
    requested_end_at = requested_start_at + timedelta(hours=hours) if requested_start_at else None
    order = AdOrder(
        user_id=user_id,
        ...
        remaining_due_at=requested_start_at - timedelta(hours=24) if requested_start_at else None,
